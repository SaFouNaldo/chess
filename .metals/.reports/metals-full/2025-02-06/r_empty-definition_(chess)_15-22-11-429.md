...
/**
  ...
  */
  ...
  // Debug prints after getting actual cells
    ...
    // Update grid
    grid = grid.updated(fromRow, grid(fromRow).updated(fromCol, Cell(fromRow, charToNumber.map((c, n) => (n, c))(fromCol), Piece.Empty, PieceColor.White)))
    grid = grid.updated(toRow, grid(toRow).updated(toCol, Cell(toRow,  charToNumber.map((c, n) => (n, c))(toCol), fromCell.piece, fromCell.pieceColor)))
